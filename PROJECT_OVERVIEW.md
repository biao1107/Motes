...
**Motes / Gym 健身搭子** 是一个面向微信小程序场景的健身社交平台，围绕“找搭子、一起练、坚持打卡、长期协作”设计。项目不仅关注用户能不能开始训练，也关注用户能不能持续训练，因此在产品层面强调匹配、协作、挑战、消息和复盘的完整闭环。

### GitHub 展示建议
- 主 README 用来快速展示项目价值、页面效果和运行方式
- 本文档适合做更完整的项目结构、模块说明和技术背景补充
